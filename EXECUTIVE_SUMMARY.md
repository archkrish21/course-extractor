...
| **2** | Grade Tracking + GPA + Billing | Weeks 7–10 | Transcript page (read-only, print button), GPA API (from plan_courses), graduation requirements with matching rules, Progress page (`/progress`, print button), dashboard (2x2 grid + compact graduation progress + active plan + full-width Validation Report card), planner validation report panel + validation indicator, plan selection persistence, Stripe integration, subscription enforcement |
