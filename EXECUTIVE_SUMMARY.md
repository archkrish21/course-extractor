...
- **Database**: 33 tables in PostgreSQL via Drizzle ORM on Supabase (local dev). All tables from the tech design doc are implemented. `semesters_offered` integer array column added to courses table. `state` and `schoolName` columns on accounts table. New `school_requests` table for school request submissions.
...
- **Tests**: 13 new API tests for consent/auth-me/accounts + 20+ new E2E tests for consent/settings/legal pages. **Total: 250 tests.**
