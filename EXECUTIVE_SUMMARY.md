...
| **2** | Grade Tracking + GPA + Billing | Weeks 7–10 | Transcript page (read-only), GPA API (from plan_courses), graduation requirements with matching rules, Progress page (`/progress`), dashboard (2x2 grid + compact graduation progress + active plan + full-width Validation Report card), planner validation report panel + validation indicator, plan selection persistence, Stripe integration, subscription enforcement |
...
**Phase 2 Grade Tracking (in progress):** Transcript page (read-only view of completed courses from the primary plan with grades, semester GPA, grade-level GPA, and cumulative GPA — replaces the previously planned Grade Tracker). GPA API calculates cumulative and projected GPA from `plan_courses` on the primary plan (not `grade_entries`). Graduation requirements rewritten with matching rules — each requirement has a `matching_rule` JSONB column supporting 5 rule types: `code_prefix` (e.g., ENG courses), `codes` (specific course codes), `division` (all courses in a division), `multi_division` (multiple divisions), and `remainder` (catch-all for unclaimed courses). Requirements API enhanced with optional `?planId=` query parameter to validate any plan, not just the primary plan. All grade entry happens in the planner page via status dropdown + grade dropdown on each course card.

- **Progress page** (`/progress`): Dedicated graduation requirements progress page showing a summary card with Total Credits, Earned, Planned, Requirements Met, and Gaps counts (gaps highlighted with warning icon in red). Overall segmented progress bar (earned/planned/remaining). Per-requirement cards with status badge, segmented progress bar, notes, and course chips (color-coded earned vs planned). Gap message showing credits still needed per requirement.
- **Dashboard layout updated**: All 4 cards in a uniform 2x2 grid (`md:grid-cols-2`). Active Plan card moved to top-left (first position via `md:-order-1`). Graduation Progress card simplified — removed per-requirement progress bars, now shows compact `earned+planned/required` format with a "View Progress" button linking to `/progress`. Quick Actions reordered to match nav (Browse Courses, Open Planner, Print Plan, View Progress, View Transcript). Credit display updated to show `earned + planned = total / required` format. New full-width **Validation Report card** below the 2x2 grid shows a "Valid" (green) or "Issues found" (red) badge. When issues exist, lists Graduation Requirement Gaps (red) and Plan Warnings (amber); when valid, shows a green success message. Uses data from the requirements API and plan validation API for the primary/active plan.
- **Navigation updated**: New "Progress" nav item added between Planner and Transcript. Menu order is now: Dashboard, Courses, Planner, Progress, Transcript, Settings.
- **Planner validation report panel**: Toggle button with check-badge icon in toolbar reveals a collapsible panel with 3 sections — Graduation Requirement Gaps (red, expanded by default), Plan Warnings (amber, expanded by default), Graduation Requirements Covered (collapsed by default). Shows summary stats: Total Credits, Earned, Planned, Requirements Met, Gaps, Warnings. Works with any selected plan (not just primary).
...
*This document summarizes the full feature specification in FEATURE_ANALYSIS.md (rev 7, April 2026). For technical schema, API design, acceptance criteria, and testing strategy, refer to the full specification.*
