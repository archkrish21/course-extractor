...
- **Database**: 34 tables in PostgreSQL via Drizzle ORM on Supabase (local dev). All tables from the tech design doc are implemented. `semesters_offered` integer array column added to courses table. `state` and `schoolName` columns on accounts table. New `school_requests` and `contact_messages` tables.
...
- **First name / last name on users**: `firstName` and `lastName` columns added to users table. Signup sets firstName from email prefix. `GET /api/v1/auth/me` returns first_name and last_name; `PATCH /api/v1/auth/me` accepts first_name/last_name updates. Layout displays full name instead of email prefix. Settings has inline name editing. Linked account members show full names.
...
- **Public homepage** (`/`): Hero with gradient text, animated stats bar, animated trial badge. "Why SAPS?" problem section. 5 feature cards with unique color accents. 3-step timeline how-it-works. FAQ accordion. Final CTA. Feature-flagged pricing and testimonials sections (dormant for v1).
- **Public layout**: Sticky navbar with glass blur, logo, nav links (About, FAQ), Sign in, Get Started Free CTA. Mobile hamburger menu. Footer with Product/Legal/Connect columns, social media icons (Instagram, Facebook, Twitter, LinkedIn), feedback mailto, school request link, copyright with disclaimer.
- **About page** (`/about`): Story, mission, what-we-do (Plan/Track/Connect cards), looking ahead, disclaimer.
- **Contact page** (`/contact`): Form with name/email/subject/message. Stored in `contact_messages` table via `POST /api/v1/contact` (no auth). Feature-flagged (dormant for v1).
- **Homepage feature config** (`config/homepage-features.ts`): `showTestimonials`, `showContactPage`, `showPricing` — all false for v1.
- **SEO**: Meta description, keywords, Open Graph tags on root layout.
- **New table**: `contact_messages` (name, email, subject, message, created_at).
- **Tests**: 17 new E2E tests for homepage/about/contact + 5 new API tests for contact endpoint. **Total: 266 tests.**
