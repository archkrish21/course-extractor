...
| GET | `/api/v1/divisions` | any | Division + department picker data, derived from the latest catalog version. Returns `[{ id, name, code, departments: [{ id, name }] }]` sorted alphabetically. Filters out summer-only divisions/departments (any course whose `semesters_offered` contains a positive value, or is NULL, qualifies the row). Cached client-side for 5 minutes via `useDivisions()`; replaces the previously hardcoded `DEPARTMENTS_BY_DIVISION` map in the courses page and course picker so newly seeded departments (e.g. Lake County Tech Campus) appear without a code change. |
