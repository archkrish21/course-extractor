...
- **Guided tour system** (refreshed in tour-UX rework): driver.js (5KB) plus shared orchestration in `lib/hooks/run-tour.ts`. Four adaptive tours: Welcome (dashboard, 7 steps including a Courses nav step), Courses (3-4 steps; mobile vs. desktop selector swap; card step dropped when filters yield no results), Planner (2-5 steps adaptive), Progress (1-3 steps adaptive). **Consent-first launch** — tours don't auto-fire; `<TourInvite />` bottom-right card asks the user to opt in (Start tour / Skip). **Forward-action endings** — the last step's "Done!" is replaced by a labelled CTA that navigates onward (Welcome → "Start planning →", Planner → "Browse courses →", Courses → "See your progress →", Progress → "Refine your plan →" / "Start a plan →"). **Interactive moments** — courses search auto-advances on input, progress filter auto-advances on click. Mobile: Tour button visible (icon-only on `< sm`); popover capped at `min(90vw, 320px)`. Tour state on `users.tourState` accepts `boolean` (legacy) or `{ completed, declined, lastStep }` via `PATCH /api/v1/auth/me`. Copy is benefit-led, school-agnostic, Genie "I" voice. Files: `lib/hooks/use-tour.ts`, `lib/hooks/run-tour.ts`, `lib/hooks/tour-state.ts`, `config/tours.ts`, `components/tour-invite.tsx`, `components/tour-button.tsx`, `data-tour` attributes on dashboard / courses / planner / progress.
