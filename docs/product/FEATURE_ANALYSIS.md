...
- **Invite role enforcement:** When an invite link is clicked, the signup form pre-selects the role from the URL and disables the other tabs ("Role set by your invite" hint shown). The signup endpoint also enforces this server-side: invite-driven signups use `invite.targetRole` as the source of truth (form-supplied role ignored), and stale invites return `400 INVITE_INVALID`. Together these prevent an invitee from accidentally signing up as the wrong role and ending up with both a self-owned student account *and* the invited membership.
- **Member-invite email lookup is case-insensitive:** `POST /api/v1/accounts/:id/members` uses `lower(users.email) = ?` for both the already-linked check and the existing-user lookup that picks the email template — `users.email` is stored as typed at signup, so a mixed-case stored email previously caused the wrong (new-user) signup link to be sent.
- **Pending invites visible in Shared With:** Sent-but-unaccepted invites now render in the Settings → Shared with list alongside accepted members, with a `Pending` badge, "Expires in N days" hint, and an X button to revoke. Backed by a new `target_email` column on `account_invite_codes` (populated lowercased on send), an extended `GET /api/v1/accounts/:id/members` response shape `{ members, pending_invites }`, and a `DELETE /api/v1/accounts/:id/invites/:inviteId` revoke endpoint. Permission to revoke matches the member-removal rule: students can revoke any invite on their account, other roles only invites they sent. The send-invite confirmation no longer shows the raw invite code; the toast names the recipient and the new pending row is the durable confirmation. Re-inviting the same email while a pending invite is outstanding is blocked with `409 ALREADY_INVITED` — the inviter must wait for acceptance or revoke the existing row first.

### Public Pages & Acquisition (Phase 3 — implemented)

- **Public homepage (`/`):** Hero section with gradient text, "For high school students & parents" audience eyebrow, animated stats bar (courses, prerequisites, requirements tracked), animated trial badge ("14-day free trial"). "Why SAPS?" problem section. 5 feature cards with unique color accents. 3-step timeline how-it-works. FAQ accordion. Final CTA ("Get Started Free"). Feature-flagged pricing section (dormant for v1 via `config/homepage.ts`: `showPricing: false`). Testimonials section dormant (`showTestimonials: false` — no real testimonials yet).
...
- **Contact page (`/contact`):** Form with name, email, subject, message fields. Submissions stored in `contact_messages` table via `POST /api/v1/contact` (no auth required). Route also sends a notification email to `planwithgenie@gmail.com` via Resend with `replyTo` set to the sender; HTML in user input is escaped server-side. Enabled via `HOME_FEATURES.showContactPage: true` in `config/homepage.ts` and linked from nav + footer.
...
- **Guided tour system** (refreshed in tour-UX rework): driver.js (5KB) plus shared orchestration in `lib/hooks/run-tour.ts`. Four adaptive tours: Welcome (dashboard, 7 steps including a Courses nav step), Courses (3-4 steps; mobile vs. desktop selector swap; card step dropped when filters yield no results), Planner (2-5 steps — 2 when no plans, 5 when plans exist), Progress (1-3 steps — 1 when no plan data, 3 when data exists). **Consent-first** — tours don't auto-fire; a `<TourInvite />` bottom-right card asks the user to opt in. **Forward CTAs** replace the "Done!" button on the last step (Welcome → "Start planning →", Planner → "Browse courses →", Courses → "See your progress →", Progress → "Refine your plan →" / "Start a plan →"). **Interactive moments** — courses search auto-advances on input, progress filter auto-advances on click. Mobile-aware: Tour button visible (icon-only `< sm`); popover capped at `min(90vw, 320px)`. Tour state on `users.tourState` is JSONB accepting `boolean` (legacy) or `{ completed, declined, lastStep }` via `PATCH /api/v1/auth/me`. Files: `lib/hooks/use-tour.ts`, `lib/hooks/run-tour.ts`, `lib/hooks/tour-state.ts`, `config/tours.ts`, `components/tour-invite.tsx`, `components/tour-button.tsx`, `data-tour` attributes on dashboard / courses / planner / progress.
...
- ✅ Public homepage (`/`): hero section with gradient text, "For high school students & parents" audience eyebrow, animated stats bar, animated trial badge, "Why SAPS?" problem section, 5 feature cards with unique color accents, 4-step timeline how-it-works (Pick your grade level / Track grad progress / Run what-if scenarios / Loop in your family), FAQ accordion, final CTA. Feature-flagged pricing section (dormant for v1). Testimonials section dormant (`showTestimonials: false` — no real testimonials yet)
...
- ✅ Contact page (`/contact`): form with name/email/subject/message, stored in `contact_messages` table via `POST /api/v1/contact` (no auth required). Sends notification email to `planwithgenie@gmail.com` via Resend with reply-to sender. Enabled in nav + footer via `HOME_FEATURES.showContactPage: true`
...
- ✅ Guided tour system (refreshed): driver.js (5KB) plus shared orchestration in `lib/hooks/run-tour.ts`. Four adaptive tours: Welcome (7 steps), Courses (3-4 steps; mobile/desktop variant), Planner (2-5 steps adaptive), Progress (1-3 steps adaptive). Consent-first via `<TourInvite />` (no auto-fire); forward-action CTAs replace "Done!" on the last step; interactive auto-advance on courses search and progress filter. Mobile: Tour button visible (icon-only `< sm`); popover capped at `min(90vw, 320px)`. Tour state on `users.tourState` is JSONB accepting boolean or `{ completed, declined, lastStep }` via `PATCH /api/v1/auth/me`. `useTour` hook, `config/tours.ts`, `data-tour` attributes, `TourButton` and `TourInvite` components.
