...
function AccountSwitcher() {
  ...
  const isParentOrCounselor = accounts.some((a) => a.role === "parent" || a.role === "counselor" || a.role === "guardian");
  const isParentLike = userRole === "parent" || userRole === "guardian";
  const showSwitcher = accounts.length >= 2 || isParentOrCounselor;
  const { onboardingCompleted } = useAccount();
  const needsOnboarding = currentAccount?.role === "student" && !onboardingCompleted;
  ...
  if (!currentAccount) {
    // User exists but has no accounts (skipped onboarding)
    const fallbackName = [userFirstName, userLastName].filter(Boolean).join(" ") || userEmail?.split("@")[0] || "User";
      ...
      <div ref={containerRef} className="relative">
        ...
        {open && (
          <div className="absolute right-0 top-full z-50 mt-2 w-48 rounded-lg border border-border bg-card shadow-lg overflow-hidden">
            {isParentLike ? (
              <Link href="/settings?add-student=1" onClick={() => setOpen(false)} className="flex w-full items-center gap-2 px-3 py-2.5 text-sm text-primary font-medium hover:bg-muted transition-colors">
              ...
              <Link href="/onboarding" onClick={() => setOpen(false)} className="flex w-full items-center gap-2 px-3 py-2.5 text-sm text-foreground hover:bg-muted transition-colors">
    ...
    return (
      <div ref={containerRef} className="relative">
        ...
        {open && (
          <div className="absolute right-0 top-full z-50 mt-2 w-48 rounded-lg border border-border bg-card shadow-lg overflow-hidden">
            {needsOnboarding && (
              <Link href="/onboarding" onClick={() => setOpen(false)} className="flex w-full items-center gap-2 px-3 py-2.5 text-sm text-primary font-medium hover:bg-muted transition-colors">
            ...
            {!FREE_LAUNCH_MODE && currentAccount?.role !== "counselor" && (
    ...
    <div ref={containerRef} className="relative">
      ...
      {open && (
        ...
        >
          ...
          {/* Add Student / Settings / Billing / Sign out */}
          <div className="border-t border-border">
            {isParentLike && (
              <Link href="/settings?add-student=1" onClick={() => setOpen(false)} className="flex w-full items-center gap-2 px-3 py-2.5 text-sm text-primary font-medium hover:bg-muted transition-colors">
                <svg aria-hidden="true" className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M18 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0ZM3 19.235v-.11a6.375 6.375 0 0 1 12.75 0v.109A12.318 12.318 0 0 1 9.374 21c-2.331 0-4.512-.645-6.374-1.766Z" /></svg>
      ...
      <footer className="border-t border-border bg-muted/30">
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-3 px-4 py-4 text-xs text-muted-foreground sm:flex-row sm:justify-between sm:px-6 lg:px-8">
          <span className="text-center sm:text-left">
            &copy; {new Date().getFullYear()} Plan with Genie. All rights reserved. Not affiliated with any school or school district.
          </span>
