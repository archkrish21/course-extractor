      ...
      <div className="print-container bg-white text-black">
        ...
        <div className="mb-4 border-b-2 border-black pb-3">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-xl font-bold">Plan with Genie — Academic planning, granted</h1>
        ...
        <div className="mb-4 flex items-center gap-6 rounded border border-border bg-muted px-4 py-2 text-sm">
          ...
          {projectedGPA.unweighted !== null && (
            <span>
              ...
              {formatGPA(projectedGPA.unweighted)}<span className="ml-0.5 text-xs opacity-80">UW</span> · {formatGPA(projectedGPA.weighted)}<span className="ml-0.5 text-xs opacity-80">W</span>
            </span>
          )}
          {actualGPA.unweighted !== null && (
            <span>
              <strong>Actual GPA:</strong>{" "}
              {formatGPA(actualGPA.unweighted)}<span className="ml-0.5 text-xs opacity-80">UW</span> · {formatGPA(actualGPA.weighted)}<span className="ml-0.5 text-xs opacity-80">W</span>
            </span>
          )}
          <span>
            ...
            <div key={grade} className="mb-4 print-grade-block">
              <div className="flex items-center gap-4 border-b border-gray-400 pb-1">
                ...
                {gradeGPA.unweighted !== null && (
                  <span className="text-xs text-muted-foreground">
                    GPA: {formatGPA(gradeGPA.unweighted)}<span className="ml-0.5 text-[10px] opacity-80">UW</span> · {formatGPA(gradeGPA.weighted)}<span className="ml-0.5 text-[10px] opacity-80">W</span>
                  </span>
                )}
              </div>
        ...
        <div className="mt-4 border-t border-border pt-2 text-[10px] text-muted-foreground">
          <p>Generated by Plan with Genie | {today}</p>
          <p>(W) = GPA waiver applied | UW = Unweighted, W = Weighted | Cr = Credits per semester</p>
